...
- Refactor: Enhance validation error handling and standardize exception arguments. Introduced prefix support in
  ...
  `Object` in exception constructors, and improved code formatting for readability. Updated exception messages and added
  a new i18n property for constraint violations.
- Reorganized multi-tenant components, replacing and renaming classes for improved structure and clarity. Introduced
  `MultiTenantContextHolder` strategies (thread-local, global) and updated schema creation with enhanced locking and
  error handling. Simplified configuration metadata and removed unused classes.
- Refactor: Simplify JMS configuration and update exception argument types. Replaced `Serializable` with `Object` in
  `JmsApplicationException` constructors for flexibility, adjusted expression handling for `responseDlqQueue`, and
  tidied imports for clarity.
- Refactor: Update logback configuration to consolidate `conversionRule` attribute naming. Replaced `converterClass`
  with `class` for consistency across logging configurations.
- Refactor: Upgrade dependency versions across modules and introduce exclusions. Updated various dependencies, plugins,
  and parent versions for improved compatibility and performance. Added BOM import for kotlinx-coroutines and introduced
  new exclusions in dependency configurations.
- Refactor: Update import paths in AutoConfigureSpringBloomDataJpa. Reorganized to reflect renamed package structure for
  improved coherence.
